...
| INTERNAL | — | 提交 | 是（平台自动重试） | contact_platform_admin | 平台内部错误，正在自动重试；若持续失败请联系 BlueMedia 支持团队并提供消息 ID。 |
...
| WABA_RESTRICTED | 131031 | 提交 | 否 | contact_platform_admin | WABA 因违反平台政策被限制或数据无法验证，需 BlueMedia 支持团队在 Meta 后台处理。 |
