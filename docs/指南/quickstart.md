...
响应 `202 Accepted`，返回消息 id 与初始状态 `accepted`。**这不代表已送达**——用 `GET /v1/messages/{id}` 或 Webhook 追踪后续状态（见[发消息与状态追踪](/docs/messaging)）。

> 📘
> **幂等：**带上 `Idempotency-Key`（≤200 字符）。在当前授权范围内重复使用同一个 key 会返回*原来那条*消息，不会重复发送——重试网络超时的请求时务必带上。

接下来建议阅读[认证与权限](/docs/authentication)了解授权范围与 scope，以及[Webhook 集成](/docs/webhook-integration)了解如何接收状态回调和用户回复。
